...
int JMeshXMLImporter :: readFaces( const string &filename, const JMeshPtr &m)
{
    ...
    while( !infile.eof() ) {
        ...
        if( str == "<Triangles>" ) {
            ...
            assert( str == "</Count>" );
            nodes.resize(3);
            JFaceSequence newtriangles = Triangle::newObjects(ncount);
        ...
        if( str == "<Quads>" ) {
            ...
            assert( str == "</Count>" );
            nodes.resize(4);
            JFaceSequence newquads = Quadrilateral::newObjects(ncount);
            for( size_t i = 0; i < ncount; i++) {
                infile >> n1 >> n2 >> n3 >> n4;
                nodes[0] =  mesh->getNodeAt(n1);
                nodes[1] =  mesh->getNodeAt(n2);
        ...
        if( str == "<Polygons>" ) {
            ...
            infile >> str;
            assert( str == "</Count>" );
            JFaceSequence newpolys = Polygon::newObjects(ncount);
            for( size_t i = 0; i < ncount; i++) {
                infile >> nnodes ;
                nodes.resize(nnodes);
...
int JMeshXMLImporter :: readCells( const string &filename, const JMeshPtr &m)
{
    ...
    while( !infile.eof() ) {
        ...
        if( str == "<Tetrahedra>" ) {
            ...
            assert( str == "</Count>" );
            nodes.resize(4);
            JCellSequence newtets = Tetrahedron::newObjects(ncount);
            for( size_t i = 0; i < ncount; i++) {
        ...
        if( str == "<Hexahedra>" ) {
            ...
            assert( str == "</Count>" );
            nodes.resize(8);
            JCellSequence newhexs = Hexahedron::newObjects(ncount);
            for( size_t i = 0; i < ncount; i++) {
        ...
        if( str == "<TriangularPrism>" ) {
            ...
            for( size_t i = 0; i < ncount; i++) {
                ...
                JCellPtr c  =  TriangularPrism::newObject();
                c->setNodes(nodes);
                c->setID( index++);
                mesh->addObject(c);
            }
